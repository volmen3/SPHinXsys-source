...
namespace SPH
{
	...
	Real ElasticSolidParticles::von_Mises_strain_static(size_t particle_i) // not tested in 2D
	{
		...
		return sqrt((1.0 / 3.0) * (powerN(epsilonxx - epsilonyy, 2) + powerN(epsilonyy - epsilonzz, 2) + powerN(epsilonzz - epsilonxx, 2)) + 2.0 * (powerN(epsilonxy, 2) + powerN(epsilonyz, 2) + powerN(epsilonxz, 2)));
	}
	//=================================================================================================//
	...
	//=============================================================================================//
	void VonMisesStress::updateRange(const blocked_range<size_t> &particle_range, Real dt)
	{
	...
	//=============================================================================================//
	void VonMisesStrain::updateRange(const blocked_range<size_t> &particle_range, Real dt)
	{
		for (size_t index_i = particle_range.begin(); index_i < particle_range.end(); ++index_i)
		{
			Mat2d F = F_[index_i];
