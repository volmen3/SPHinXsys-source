...
std::vector<Vecd> CreateInflowBufferShape()
{
	...
	inlfow_buffer_shape.push_back(Vecd(-DL_sponge + 5 * resolution_ref, DH));
	inlfow_buffer_shape.push_back(Vecd(-DL_sponge + 5 * resolution_ref, 0.0));
	inlfow_buffer_shape.push_back(Vecd(-DL_sponge, 0.0));
...
{
	std::vector<Vecd> outflow_buffer_shape;
	outflow_buffer_shape.push_back(Vecd(DL, -10.0*BW));
	outflow_buffer_shape.push_back(Vecd(DL, DH+10.0*BW));
	outflow_buffer_shape.push_back(Vecd(DL + 10.0*BW, DH+10.0*BW));
	outflow_buffer_shape.push_back(Vecd(DL + 10.0*BW, -10.0*BW));
	outflow_buffer_shape.push_back(Vecd(DL, -10.0*BW));
...
std::vector<Vecd> createInnerWallShape()
{
	...
	inner_wall_shape.push_back(Vecd(-DL_sponge, 0.0));
	inner_wall_shape.push_back(Vecd(-DL_sponge, DH));
...
/** inflow buffer */
class InflowBuffer : public BodyPartByParticle
{
public:
...
class ParabolicInflow : public fluid_dynamics::EmitterInflowCondition
{
	Real u_ave_, u_ref_, t_ref;
public:
	ParabolicInflow(FluidBody* fluid_body,
		BodyPartByParticle* constrained_region)
		: EmitterInflowCondition(fluid_body, constrained_region)
	{
		u_ave_ = 0.0;
		u_ref_ = 1.0;
		t_ref = 2.0;
	...
	void setupDynamics(Real dt = 0.0) override
	{
		...
		//std::cout << "u_ave_ = " << u_ave_ << std::endl;
	}
...
{
public:
	OutletCollector(BaseParticles &base_particles) : base_particles_(base_particles)
	{
		/** Geomtry definition. */
		std::vector<Vecd> outletflow_buffer_shape = CreateOutflowBufferShape();
		buffer_shape_ = new ComplexShape("outlet");
		buffer_shape_->addAPolygon(outletflow_buffer_shape, ShapeBooleanOps::add);
	};
	void parallel_exec() 
	{
